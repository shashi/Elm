...
Elm.Basics.make = function (elm)
                  {
                    ...
                    var uncurry = F2(function (f,arg0)
                                     {
                                       return function ()
                                              {
                                                switch (arg0.ctor)
...
Elm.DataSource.make = function (elm)
                      {
                        elm.DataSource = elm.DataSource || {};
                        if (elm.DataSource.values)
                        return elm.DataSource.values;
                        var N = Elm.Native,
                            _N = N.Utils.make(elm),
                            _L = N.List.make(elm),
                            _E = N.Error.make(elm),
                        ...
                        var _op = {};
                        var sessionSet = Native.DataSource.sessionSet;
                        var sessionGet = Native.DataSource.sessionGet;
                        var query = Native.DataSource.query;
                        var collection = Native.DataSource.collection;
                        elm.DataSource.values = {_op: _op, collection: collection, query: query, sessionGet: sessionGet, sessionSet: sessionSet};
                        return elm.DataSource.values;
